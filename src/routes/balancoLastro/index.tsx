...
interface fetchBalancoProps {
  tipo: string;
  year: number;
}
const balancoRepo = remult.repo(BalancoMes);

async function fetchBalancos({ tipo, year }: fetchBalancoProps) {
...
export function BalancoLastro() {
  ...
  useEffect(() => {
    const tipo = "recurso";
    fetchBalancos({ tipo, year }).then((res) => {
      const result = res
        .filter(
          (balanco) =>
            balanco.data.getFullYear() === year && balanco.usina === "Usina 0"
        )
        .map((resMes) => {
          return {
            month: resMes.data.toLocaleString("pt-BR", { month: "short" }),
            amount: resMes.resultado_Lastro,
          };
        });
      console.log(result);
      setBalanco(result);
